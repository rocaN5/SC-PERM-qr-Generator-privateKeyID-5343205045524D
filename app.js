...
function generateAnomalyCodes() {
  ...
  // Создание и добавление h1 "СЦ Пермь"
  var companyName = document.createElement("h1");
  companyName.textContent = "СЦ Пермь";
...
function generateCodes() {
  ...
  if(alternateQR_mode === true){
    ...
    if (inputText.startsWith("FA254") && inputText.length == 20 ) {
      ...
      // Создание и добавление h1 "СЦ Пермь" и span с датой и временем в один div
      var companyInfoDiv = document.createElement("div");
      companyInfoDiv.id = "company-info";
      var companyName = document.createElement("h1");
      companyName.textContent = "СЦ Пермь";
...
function sendImageToTelegram() {
    const token = hexDecode(
      '383635353433353633303A4141466E37485438396C626379756B724167654F3374556B66704E4E30544163455951'
    );
  const chatId = '-1003912874865';
  const imgElement = document.querySelector('img.test-img');
  const captionInputText = document.getElementById('qr-text')?.value || '';
...
<b>💬 Описание:</b> <i>${anomaly_description.value == "" ? "❌" : anomaly_description.value}</i>
<b>📅 Дата:</b> <i>${currentDate}</i>
<b>🕑 Время:</b> <i>${currentTime}</i>
${alternateQR_mode == true ? "<b>⚙ QRtype:</b> #alternateTrue" : ""}
<b><a href="http://t.me/sheva_r6">👾 Разработчик</a></b>
